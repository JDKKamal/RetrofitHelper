...
public class ApiClient {
    ...
    public static ApiInterface getClient(boolean with_header, final Map<String, String> customHeaders) {
        if (TextUtils.isEmpty(BASE_URL)) BASE_URL = "";
        OkHttpClient client = new OkHttpClient.Builder().connectTimeout(2, TimeUnit.MINUTES).readTimeout(2, TimeUnit.MINUTES).build();
        Retrofit retrofit;
        Retrofit.Builder builder = new Retrofit.Builder().baseUrl(BASE_URL).client(client).addConverterFactory(GsonConverterFactory.create());
    ...
    public static void setCommonHeaders(final Map<String, String> commonHeaders) {
        if (commonHeaders != null && !commonHeaders.isEmpty()) default_headers = commonHeaders;
    }
